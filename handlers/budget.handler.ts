...
export const handleBudgetInput = async (update: TelegramUpdate, user: User) => {
    ...
    if (isNaN(budgetAmount) || budgetAmount <= 0) {
        return sendMessage(chatId, "❌ ကျေးဇူးပြု၍ မှန်ကန်သော ပမာဏကို ဂဏန်းအမှန်ဖြင့်သာ ပြန်လည်ရိုက်ထည့်ပေးပါ။");
    }
    await setBudget(user.id, budgetAmount);
    await updateState(user.id, SessionState.IDLE);
    return sendMessage(chatId, `✅ ယခုလအတွက် လစဉ် အသုံးစရိတ်ကို **${formatCurrency(budgetAmount)}** အဖြစ် သတ်မှတ်ပေးလိုက်ပါပြီ။ 💪`);
};
...
export const askForBudget = async (update: TelegramUpdate, userId: string) => {
    ...
    return sendMessage(chatId, "💰 ကျေးဇူးပြု၍ သင်သတ်မှတ်လိုသော လစဉ် Budget ပမာဏကို ဂဏန်းသီးသန့်ဖြင့် ရိုက်ထည့်ပေးပါ (ဥပမာ- 300000)။");
};
