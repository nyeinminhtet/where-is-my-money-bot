...
export const handleUndo = async (update: TelegramUpdate, user: User) => {
    ...
    if (callbackData === "UNDO_LAST") {
        if (callbackQuery?.id) {
            await answerCallbackQuery(callbackQuery.id, "⚠️ ဒီစာရင်းက စနစ်မပြောင်းခင်က စာရင်းအဟောင်းဖြစ်တဲ့အတွက် Bot ထဲကနေ လှမ်းဖျက်လို့ မရတော့ပါ။", true);
        }
        return;
    }
    ...
    if (!transaction) {
        if (callbackQuery?.id) {
            await answerCallbackQuery(callbackQuery.id, "⚠️ ဒီစာရင်းက ဖျက်ပြီးသား ဖြစ်နေပါသည် (သို့မဟုတ် မရှိတော့ပါ)၊", true);
        }
        return;
    }
    ...
    if (callbackQuery?.id) {
        await answerCallbackQuery(callbackQuery.id, `🗑️ ${transaction.description || typeText} (${formatCurrency(transaction.amount)}) စာရင်းဖျက်လိုက်ပါပြီ!`, false);
    }
};
